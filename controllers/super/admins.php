...
class admins 
{
	...
	function unblock()
	{
		...
		if(!isset($_POST['but']))
		{
			...
			GSMS::load('unblock','super_view','admins');
			
		}
		else
		{
			$inf=array('page_title'=>'خروج از بلاک');
			GSMS::$class['template']->header($inf);
			$inf=array('page_title'=>'خروج از بلاک ');
			GSMS::$class['template']->panel_header($inf);
			
			$tempAdmin=null ; 
			$admin_id = intval(GSMS::$class['input']->post('admin_id'));
			if ($admin_id > 0 )
				$tempAdmin=& admin::getAdmin($admin_id);
			else 
				$tempAdmin=& admin::getAdminByUsername(GSMS::$class['input']->post('mobile'));
			
			 if(!is_object($tempAdmin))
			 {
				 $body .='<div class="alert alert-warning">شماره کاربر معتبر نیست . <br>کاربر پیدا نشد .</div><br><a class="back_btn" href="'.
						GSMS::$class['template']->info['super_url'].'">برگشت</a>';
			 }
			 else 
			 {
				$tempAdmin->is_block=0;	
				$result=$tempAdmin->save();
				
				$body .='<div class="alert alert-info">کاربر با موفقیت از بلاک خارج شد . </div><br><a class="back_btn" href="'.
						GSMS::$class['template']->info['super_url'].'">برگشت</a>';
			 }
			
			$inf=array('title'=>' خروج از بلاک','body'=>$body,'dir'=>'ltr');
	...
	function edit_admin($adminid)
	{
		...
		if(!isset($_POST['but']))
		{
			...
			GSMS::load('edit_admin','super_view','admins',$tempAdmin);
		...
		else
		{
			...
			$body .='<br><a class="back_btn" href="'.
						GSMS::$class['template']->info['super_url'].
	...
	function view_admin($adminid)
	{
		...
		GSMS::load('view_admin','super_view','admins',$tempAdmin);
	...
	function list_admins($begin=0,$end=0)
	{
		...
		GSMS::load('list_admin','super_view','admins',$tempAdmins);
	}//func
	...
	function change_password()
	{
		...
		if($oldpass=='')
		{
			GSMS::load('change_password','super_view','admins');
		}
		else
		{
