...
class v1
{
		...
		if (isset($_FILES)) 
		{
			...
			if (!file_exists(GSMS::$tempDir . $fileName)) 
			{
				...
				if (move_uploaded_file($_FILES['uploaded_file']['tmp_name'], GSMS::$tempDir . $fileName)) 
				{
					...
					$path .= $this->user->id . DIRECTORY_SEPARATOR;
					$pathDB  = $this->user->id . DIRECTORY_SEPARATOR;
    ...
    function insert()
    {
		...
		if (intval($data['info']["is_online"])>0 )   
		{
			...
			{
				echo json_encode(array('شجره نامه پایه پیدا نشد .')); 
				//block user 
				//try to edit 
				return ; 
			}
			...
			if ($shajarename->userId != $this->user->id )
			{
				...
				$this->user->save() ; 
				echo json_encode(array('خطای اهراز حویت . دسترسی کاربران متخلف بسته می شود .')); 
				return;
			}
		...
		if( $this->user->is_block > 0  ) 
		{
			...
			else
			{
				...
				echo json_decode(array( "عدم دسترسی به بخش ثبت شجره نامه به علت تخلف .\n  از منوی کنار با پشتیبانی تماس بگیرید . "));
		...
		if( $this->user->is_block <= 0  ) 
		{
			...
			$geraph->createDate=GSMS::$class['calendar']->now();
	...
	function insertComment($itemId=0)
	{
		...
		$tempComment->createDate= 
				 (isset($_POST['date'])? 
					GSMS::$class['input']->post('date') : 
					GSMS::$class['calendar']->now()
					);
