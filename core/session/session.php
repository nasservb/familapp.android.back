...
//class for control session on model layer start 3-2-91 by nasser niazy in 
class session 
{
	...
	{
		ini_set('session.save_path',GSMS::$rootDir.'archive/'.$this->session_save_path);
	...
	{
		if(isset($_SESSION['login_UserType']) && ($this->get('login_UserType')==1))
		{
			return true;
		}
		return false;
	...
	public function checkLogin()
	{
		...
		if ( 	($this->is_register('login_login')) && 
				...
				($this->is_register("login_UserName")) ) 
			$login=true;
		return $login;										 
	}//func
	...
	{
		$row='';$max=0;
		GSMS::$class['DB']->run("select * from `users` where (`username`='".$username."')",'session.php',$username,$row,$max,'check pass');
		if($max==0)
		{
			return -1;
		}elseif($max>1)
		{
			GSMS::$class['DB']->logsql("select * from `users` where (`username`='".$username."')"
