...
class editTelchannel
{
    function editTelchannel($parameter)
    {
		
        $inf = array('page_title' => 'ویرایش کانال');
        ...
        for ($i = 0; $i < count($categorys); $i++)
            $category_body .= '<option value="' . $categorys[$i]['id'] . '" '.($categorys[$i]['id'] ==$parameter['group']->telgroupCategoryId ? ' selected ' : '' ) .'>' .
        ...
        for ($i = 0; $i < count($categorys); $i++)
            $ostan_body .= '<option value="' . $categorys[$i]['id'] . '"
									'.($categorys[$i]['id'] ==$parameter['group']->ostanId ? ' selected ' : '' ) .'>' .
		...
		<form action="' . GSMS::$class['template']->info['super_url'] . 'telgroups/editTelchannel" method="post" enctype="multipart/form-data">
		<div id="notif"></div>
       <table dir="rtl">
	 
        <tr><td>نام کانال	</td>
		<td>
			<input type="hidden" name="id"  value="'.$parameter['group']->id.'" />
			<input type="text" name="name" id="name" value="'.$parameter['group']->title.'" />
		</td></tr>
        <tr><td>متن کوتاه درباره کانال:</td>
		<td><textarea type="text" name="description" id="description" >'.$parameter['group']->description.'</textarea></td></tr>
        <tr><td>آدرس کانال : شبیه(https://telegram.me/)</td>
		<td><input type="text" name="address" id="address"  value="'.$parameter['group']->telegramAddress.'"/></td></tr>
        <tr>
			<td>دسته بندی</td>
			<td>
		...
		<td><input type="text" name="tag" id="tag"  value="'.$parameter['group']->tag .'"/></td></tr>
        <tr><td>استان </td>
		<td>
		...
		<td><input type="text" name="owner_mobile" id="owner_mobile"  value="'.$parameter['group']->ownerMobile.'"/></td></tr>
        <tr><td>  تعداد اعضاء</td>
		<td><input type="text" name="member" id="member"  value="'.$parameter['group']->memberCount.'" /></td></tr>
		...
		کاور بزرگ کانال</td><td><input  title="در صفحه کانال این تصویر به عنوان پس زمینه استفاده می شود" type="file" name="cover" id="cover" />
					<img src="'.GSMS::$class['template']->info['index_url'].'coverGroupView/'.$parameter['group']->coverPictureId.'"/>
					</td></tr>
		...
		تصویر کوچک کانال</td><td><input  title="در لیست کانال هااین تصویر به عنوان پس زمینه استفاده می شود" type="file" name="icon" id="icon" />
					<img src="'.GSMS::$class['template']->info['index_url'].'iconGroupView/'.$parameter['group']->iconPictureId.'"/>
		</td></tr> 
		
		<tr><td>  تاریخ ایجاد کانال </td>
		<td><input type="text" name="createDate" id="createDate"  value="'.$parameter['group']->createDate.'" /></td>
		</tr>
		
        <tr><td><input type="submit" class="btn btn-success btn-register" name="send" id="send" value="ویرایش " /></tr>
		</table>
        </form><br/>
        ...
        $inf = array('title' => 'ویرایش کانال ', 'body' => $body, 'dir' => 'ltr');
