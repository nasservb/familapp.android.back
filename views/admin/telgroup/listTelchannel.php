    ...
    function listTelchannel($tempGroupArray)
    {
        ...
        for ($i = 0; $i < count($tempGroup); $i++) {
			...
			'.
				...
				'<a target="_blank" class="btn btn-success" href="' . 
					GSMS::$class['template']->info['admin_url'] . 'telgroups/accept/' . ($tempGroup[$i]->id*34) . '">تاييد</a>' 
				)
				.  '
				<a class="btn btn-primary" href="' . GSMS::$class['template']->info['admin_url'] . 'telgroups/editTelchannel/' .
		...
		$body .='</div>
		  <br/>'. GSMS::$class['template']->paging(
            GSMS::$class['template']->info['admin_url'] .  'telgroups/listTelchannel/', $begin, $itemCount);
